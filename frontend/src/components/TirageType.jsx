...
const TirageType = ({ onNext, onBack }) => {
  ...
  const tirages = [
    ...
    { label: 'Tirage 6 runes : Question approfondie', prix: 45 },
  ];
  ...
  const addCustomQuestion = () => {
    const maxQuestions = selectedType.includes('6 runes') ? 6 : 4;
    
    if (customQuestions.length >= maxQuestions) {
      alert(`Vous ne pouvez écrire que ${maxQuestions} questions maximum.`);
      return;
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedType.includes('6 runes')) {
      // cas tirage 6 runes → thème + questionnement libre
      if (!theme || !mainQuestion.trim()) {
        alert('Veuillez choisir un thème et décrire votre questionnement.');
        return;
      }
      onNext({
    ...
    } else if (selectedType.includes('4 runes')) {
      // cas tirage 4 runes → thème + questions personnalisées obligatoires
      const totalQuestions = customQuestions.filter(q => q.trim() !== '').length;
      ...
      if (totalQuestions === 0) {
        alert('Veuillez écrire au moins une question personnalisée (maximum 4).');
        return;
      }
  ...
  return (
    <form className="tirage-type-form" onSubmit={handleSubmit}>
      ...
      {/* Bloc des questions */}
      {selectedType && selectedType.includes('6 runes') ? (
        <div className='question-section theme-section'>
          <h3>Votre questionnement approfondi</h3>
          ...
          <div className="info-box">
            <p><strong>ℹ️ Tirage 6 runes - Question approfondie</strong></p>
            <p>Décrivez votre questionnement et votre situation. Ce tirage développera votre questionnement à travers 6 runes selon la structure suivante :</p>
            <p><strong>Passé → Présent → Obstacle → Aide → Futur → Conseil</strong></p>
          </div>

          <label>Décrivez votre questionnement et votre situation :</label>
          <textarea
            value={mainQuestion}
            onChange={(e) => setMainQuestion(e.target.value)}
            placeholder="Expliquez votre situation, votre questionnement, et ce que vous souhaitez éclaircir ou comprendre..."
            rows="6"
            required
          />
        </div>
        ...
        <div className='question-section'>
          ...
          <div className="info-box">
            ...
            <p>Pour le tirage à 4 runes, vous devez écrire vos propres questions personnalisées (4 maximum).</p>
            <p>Les questions ci-dessous sont des <strong>exemples</strong> pour vous inspirer.</p>
          </div>

          {/* Questions exemples */}
          <div className="example-questions">
            <h4>📋 Exemples de questions (pour inspiration uniquement)</h4>
            {Object.entries(questionCategories).map(([category, questions]) => (
              <div key={category} className="question-category example">
                <button
                  type="button"
          ...
          <div className="custom-questions-section">
            <h4>✍️ Vos questions personnalisées ({customQuestions.filter(q => q.trim()).length}/{selectedType.includes('6 runes') ? '6' : '4'})</h4>
            {customQuestions.map((question, index) => (
              <div key={index} className="custom-question">
                ...
                <input
                  type="text"
                  value={question}
                  placeholder={`Écrivez votre question ${index + 1}`}
                  onChange={(e) => handleCustomQuestionChange(index, e.target.value)}
                  required
                />
                <button type="button" className="remove-btn" onClick={() => removeCustomQuestion(index)}>✕</button>
              </div>
            ))}
            {(() => {
              const maxQuestions = selectedType.includes('6 runes') ? 6 : 4;
              return customQuestions.length < maxQuestions && (
                <button type="button" className="add-question-btn" onClick={addCustomQuestion}>
                  + Ajouter une question ({customQuestions.length}/{maxQuestions})
                </button>
              );
            })()}
          </div>
